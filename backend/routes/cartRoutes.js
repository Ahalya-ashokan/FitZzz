  ...
  try {
    ...
    if (cart) {
      ...
      if (productIndex > -1) {
        ...
        // if the product is not present in the cart, just add new product
        cart.products.push({
          productId,
          name: product.name,
...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    if (cart) {
      ...
      res.status(404).json({ massage: "Cart no found" });
    }
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Server Error" });
      ...
      if (userCart) {
        ...
        guestCart.products.forEach((guestItem) => {
          ...
          if (productIndex > -1) {
            ...
            // otherwise, simply add the guest item to the cart
